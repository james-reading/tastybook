...
RSpec.describe FriendRequestForm do
  ...
  let(:holly) { create :friend }

  let(:form) { FriendRequestForm.new(user: james, friend_id: holly.id) }
  ...
  it 'should create a pending friend request' do
    ...
    expect(friend_request.friend).to eq holly
  end
  ...
  context 'user has already invited the friend' do
    ...
    it 'should reuse the existing friendship' do
      expect {
        form.submit
